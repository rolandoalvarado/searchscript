# Date: Oct. 24, 2013
# Description: This includes the client-side validations for User Registration.
$(document).ready ->
  ...
  validateForm = () ->
    $.extend(badFields = [], validateInputs(inputs, emptyRegEx), validateInputs(emails, emailRegEx), validateInputs(codes, postalCodeRegEx), validateSelect(selects), validateInputs(numbers, numberRegEx), validateChoiceSelect(choices), validateCheckbox(checkboxes))
  ...
  validateCheckbox = (checkboxes) ->
    error = []
    for checkbox in checkboxes
      if $(checkbox).hasClass 'checked'
        removeErrorStyleCheckbox(checkbox)
      else
        $(checkbox).removeClass 'checked'
        error.push($(checkbox).attr("id"))
        addErrorStyleCheckbox(checkbox)
    return error
      
  addErrorStyle = (element) ->
    $(element).addClass('form-error')    
    $(element).prev('label').find('.error_message').html(errorMessages[$(element).attr("id")])
      
  removeErrorStyle = (element) ->
    $(element).removeClass('form-error')
    $(element).prev('label').find('.error_message').html("")
  
  addErrorStyleCheckbox = (element) ->
    $(element).addClass('form-error')    
    $(element).prev('input').prev('label').find('.error_message').html(errorMessages[$(element).attr("id")])
  
  removeErrorStyleCheckbox = (element) ->
    $(element).removeClass('form-error')
    $(element).prev('input').prev('label').find('.error_message').html("")
